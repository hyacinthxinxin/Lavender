...
- (void)uploadWithOperator:(NSString *)operatorName
    ...
    _httpClient = [UpSimpleHttpClient POST:urlString
                         ...
                         sendProgressBlock:^(NSProgress *progress) {
                             self->_totalUnitCountToSend = progress.totalUnitCount;
                         ...
                         completionHandler:^(NSError *error, id response, NSData *body) {
                             ...
                             if (res.statusCode >= 400) {
                                 if (!error) {
                                     error  = [[NSError alloc] initWithDomain:NSErrorDomain_UpYunFormUploader code:res.statusCode userInfo:NULL];
                                 }
                                 failureBlock(error, res, retObj);
                                 return ;
                             }
                             // 上传成功
                             progressBlock(self->_totalUnitCountToSend, self->_totalUnitCountToSend);//使发送进度为 100%
