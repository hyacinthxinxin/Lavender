    ...
    dispatch_async(_uploaderQueue, ^(){
        if (self->_failureBlock) {
            ...
            NSError * error  = [[NSError alloc] initWithDomain:NSErrorDomain_UpYunBlockUpLoader
                                                          code: -101
                                                      userInfo: userInfo];
            self->_failureBlock(error, nil, nil);
        }
        [self clean];
    });
}
    ...
    _httpClient = [UpSimpleHttpClient PUT:urlString
                        ...
                        completionHandler:^(NSError *error, id response, NSData *body) {
                            ...
                            if (res.statusCode == 204) {
                                ...
                                self->_next_part_id = [next_part_id intValue];
                                
                                if (self->_progressBlock && self->_next_part_id >= 0) {
                                    self->_progressBlock(self->_next_part_id * UpYunFileBlcokSize, self->_fileSize);
                                }
                                self->_X_Upyun_Multi_Uuid = [resHeaders objectForKey:@"x-upyun-multi-uuid"];
                                ...
                                if (self->_failureBlock) {
                                    ...
                                    self->_failureBlock(error, response, retObj);
                                    [self clean];
...
//    NSLog(@"_next_part_id %d", _next_part_id);
    ...
    _httpClient = [UpSimpleHttpClient PUT:urlString
                        ...
                        sendProgressBlock:^(NSProgress *progress) {
                            if (self->_progressBlock && self->_next_part_id >= 0) {
                                self->_progressBlock(self->_next_part_id * UpYunFileBlcokSize + progress.completedUnitCount, self->_fileSize);
                            }
                        }
                        completionHandler:^(NSError *error, id response, NSData *body) {
                            NSHTTPURLResponse *res = response;
                            ...
                            if (res.statusCode == 204) {
                                ...
                                if (returnDetailCode == 40011059 || returnDetailCode == 40011062) {
                                    //文件已经存在，块已经存在
                                    NSLog(@"%d", returnDetailCode);
                                    self->_next_part_id = self->_next_part_id + 1;
                                    dispatch_async(self->_uploaderQueue, ^(){
                                        if (self->_cancelled) {
                                            [self canceledEnd];
                                        } else {
                        ...
                        completionHandler:^(NSError *error, id response, NSData *body) {
                            ...
                            if (res.statusCode == 204 ||
                                ...
                                if (self->_progressBlock) {
                                    self->_progressBlock(self->_fileSize, self->_fileSize);
                                    
                                }
                                
                                if (self->_tasks.count > 0 && self->_notify_url. length > 0) {
                                    UpYunFileDealManger *upDeal = [[UpYunFileDealManger alloc] init];
                                    [upDeal dealTaskWithBucketName:self->_bucketName operator:self->_operatorName password:self->_operatorPassword notify_url:self->_notify_url source:self->_savePath tasks:self->_tasks success:self->_successBlock failure:self->_failureBlock];
                                } else {
                                    if (self->_successBlock) {
                                        self->_successBlock(response, retObj);
                                    }
                                }
