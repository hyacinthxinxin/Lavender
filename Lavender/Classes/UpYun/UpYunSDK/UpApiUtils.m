...
+ (NSString *)getSignatureWithPassword:(NSString *)password parameters:(NSArray *)parameter {
    /*Signature 计算方式
    ...
    NSString *passwordHash = [UpApiUtils getMD5HashFromData:[NSData dataWithBytes:password.UTF8String length:password.length]];
    NSString *signature = [UpApiUtils getHmacSha1HashWithKey:passwordHash string:parameterString];
    if (!signature) {
